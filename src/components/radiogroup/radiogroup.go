...
var (
	//go:embed radiogroup.html
	radiogroupHTML string
	radiogroupTpl  = component.New("radiogroup.html", radiogroupHTML)
...
// it backs the hidden radio input and its label's for attribute.
type Option struct {
	Id      string
	Value   string
	Label   string
...
// Options configures a single-choice group of up to about 14 options,
// rendered as a segmented row of pills in the style of tabs. Name is the form
// field name shared by every option's radio input.
type Options struct {
	Name     string
	Label    string
	Options  []Option
...
func Render(options Options) (template.HTML, error) {
	...
	groupId := "radiogroup-" + options.Name
	hintId := groupId + "-hint"
	...
	return radiogroupTpl.Render(opts)
}
